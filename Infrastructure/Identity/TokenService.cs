...
internal class TokenService : ITokenService
{
    ...
    public async Task<TokenResponse> GetTokenAsync(TokenRequest request, string ipAddress, CancellationToken cancellationToken, string requestedTenant)
    {
    ...
    public IEnumerable<Claim> GetClaims(string userId, string email, string firstName, string lastName, string imageUrl, string phoneNumber, string ipAddress) =>
        new List<Claim>
        {
            new(ClaimTypes.NameIdentifier, userId),
            new(ClaimTypes.Email, email),
            new(FSHClaims.Fullname, $"{firstName} {lastName}"),
            new(ClaimTypes.Name, firstName ?? string.Empty),
            new(ClaimTypes.Surname, lastName ?? string.Empty),
            new(FSHClaims.IpAddress, ipAddress),
            new(FSHClaims.ImageUrl, imageUrl ?? string.Empty),
    ...
    private string GenerateEncryptedToken(SigningCredentials signingCredentials, IEnumerable<Claim> claims, DateTime expireDate)
    {
        var token = new JwtSecurityToken(
           claims: claims,
           expires: expireDate,
