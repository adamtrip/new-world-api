...
internal class TenantService : ITenantService
{
    ...
    {
        var currentTenant = await _db.TenantInfo.FindAsync(request.Id);
        currentTenant.Name = request.Name;
        currentTenant.ConnectionString = request.ConnectionString;

        _db.TenantInfo.Update(currentTenant);
        int result = await _db.SaveChangesAsync();

        return (result == 1 ? "Tenant updated successfully" : "Error updating the tenant");
    }
}