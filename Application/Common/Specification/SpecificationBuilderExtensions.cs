...
public static class SpecificationBuilderExtensions
{
        ...
        Filter? filter)
    {
        if (filter is not null)
        {
            ...
            {
                if (filter.Filters is null) throw new Exception("The Filters attribute is required when declaring a logic");
        ...
        string filterOperator)
    {
        return filterOperator switch
        {
            ...
            _ => throw new Exception("Filter Operator is not valid."),
        };
    }
        ...
        Type propertyType)
    {
        ...
        if (propertyType.IsEnum)
        {
            ...
            if (!Enum.TryParse(propertyType, stringEnum, true, out object? valueparsed)) throw new Exception(string.Format("Value {0} is not valid for {1}", value, field));
        ...
        if (propertyType == typeof(Guid))
        {
            ...
            if (!Guid.TryParse(stringGuid, out Guid valueparsed)) throw new Exception(string.Format("Value {0} is not valid for {1}", value, field));
    ...
    {
        if (string.IsNullOrEmpty(filter.Field)) throw new Exception("The field attribute is required when declaring a filter");
        if (string.IsNullOrEmpty(filter.Operator)) throw new Exception("The Operator attribute is required when declaring a filter");
        return filter;
    }
